/*  linkparser.c  (v2.1-2025-08-03)
 *  -------------------------------------------------------------
 *  Same INI output as v2.0, plus:
 *      • one-line JSON file for /status  (default /tmp/sta_status.json)
 *      • JSON includes staX_retry, txpwr, rxinfo_* fail counters.
 */
...
/* --------------- INI ------------------- */
static int load_cfg(const char *file,char macs[][MAC_LEN],int *intv,
                    char *out,size_t os,char *status,size_t ss,char *proc,size_t ps)
{
    ...
    while(fgets(l,sizeof l,fp)){
        ...
        else if(!strcmp(k,"status_file"))                               strncpy(status,v,ss);
...
static void parse_proc(const char *path,char macs[][MAC_LEN],int n,StaStats st[],RxInfo *rx)
{
    ...
    while(fgets(line,sizeof line,fp)){
        if(strstr(line,"STA [")){
            char mac[MAC_LEN]; if(sscanf(line,"%*[^[][%17[^]]",mac)==1) cur=find_mac(mac,macs,n); else cur=-1;
            continue;
        }
        if(strstr(line,"Rx Info dump")){ cur=-1; continue; }

        if(cur==-1){
            if(strstr(line,"rssi_min")){ char *p=strstr(line,"rssi_min"); if(p)rx->rssi_min=first_int(p);}
            else if(strstr(line,"cnt_cck_fail")){
                char *p=strstr(line,"cnt_cck_fail"); rx->cnt_cck_fail=first_int(p);
                p=strstr(line,"cnt_ofdm_fail"); if(p)rx->cnt_ofdm_fail=first_int(p);
...
/* --------------- INI output ------------- */
static void write_ini(const char *file,StaStats st[],int n,int tx,const RxInfo *rx,int active)
{
    ...
    fprintf(f,"active_sta=%d\nactive_sta_rssi=%d\n",active,active>=0?st[active].rssi:-1);
    fprintf(f,"txpwr=%d\n"
...
/* --------------- JSON output ------------ */
static void write_json(const char *file,StaStats st[],int n,int tx,const RxInfo *rx,int active)
{
    char tmp[256]; snprintf(tmp,sizeof tmp,"%s.tmp",file);
    FILE *f=fopen(tmp,"w"); if(!f){ if(verbose)perror(tmp); return; }

    fprintf(f,"{\"active_sta\":%d,",active);
    for(int i=0;i<n;++i){
        fprintf(f,"\"sta%d_retry\":%d%s",i,st[i].retry,(i==n-1?",":" ,"));
    }
    fprintf(f,"\"txpwr\":%d,",tx);
    fprintf(f,"\"rxinfo_cnt_cck_fail\":%d,",rx->cnt_cck_fail);
    fprintf(f,"\"rxinfo_cnt_ofdm_fail\":%d,",rx->cnt_ofdm_fail);
    fprintf(f,"\"rxinfo_false_alarm\":%d}",rx->false_alarm);
    fclose(f); rename(tmp,file);
}
...
static void summary(const StaStats s[],int n,int tx,const RxInfo *rx,int active){
    for(int i=0;i<n;++i)
        fprintf(stderr,"%cSTA%d %s R=%d M=%s BW=%d Re=%d\n",
                i==active?'*':' ',i,s[i].mac,s[i].rssi,s[i].mcs,s[i].bw,s[i].retry);
    fprintf(stderr,"RX FA=%d CCK=%d OFDM=%d\nTX=%d dBm\n",
            rx->false_alarm,rx->cnt_cck_fail,rx->cnt_ofdm_fail,tx);
}

/* ---------------- main ----------------- */
static void usage(const char *p){ fprintf(stderr,"Usage: %s [-c cfg] [-o ini] [-s json] [-i ms] [-p proc] [-d iface] [-v]\n",p); }

int main(int argc,char *argv[])
{
    const char *cfg="/etc/sta_monitor.conf";
    char ini[256]="/tmp/sta_data.info";
    char json[256]="/tmp/sta_status.json";
    ...
    int o,i; while((o=getopt_long(argc,argv,"c:o:s:i:p:d:vh",lo,&i))!=-1){
        if(o=='c')cfg=optarg; else if(o=='o')strncpy(ini,optarg,sizeof ini);
        else if(o=='s')strncpy(json,optarg,sizeof json);
    ...
    int n=load_cfg(cfg,macs,&intv,ini,sizeof ini,json,sizeof json,proc,sizeof proc);
    ...
    for(;;){
        ...
        write_ini(ini,st,n,tx,&rx,active);
        write_json(json,st,n,tx,&rx,active);
