/* wifi_sniff2udp.c  –  Wi-Fi monitor-mode sniffer → UDP forwarder (fire-and-forget)
 *
 * Builds with:
 *   gcc -O3 -march=native -Wall -std=gnu11 -o wifi_sniff2udp wifi_sniff2udp.c -lpcap
 *
 * Forwarding is “fire-and-forget”: the UDP socket is **not connected**;
 * each mmsghdr already contains the destination sockaddr, so the kernel
 * never returns ECONNREFUSED even if nobody is listening.
 */
...
/* ---------- forward declaration (needed by handle_pkt) ------------------- */
...
static void handle_pkt(const struct pcap_pkthdr *h, const uint8_t *p)
{
    ...
    if (pres & (1 << 1)) {
        ...
        while (pres & 0x80000000) {
            if (o + 4 > h->caplen) return;
    ...
    /* accept either STA→AP (To DS) or AP→STA (From DS) */
    int tods   = (fc16 >> 8) & 1;
    int fromds = (fc16 >> 9) & 1;
    if (tods == fromds) return;          /* ignore IBSS / WDS */

    const uint8_t *addr2 = p + off + 10; /* transmitter */
    if (memcmp(addr2, mac_bssid, 6) != 0) return;

    const uint8_t *addr1 = p + off + 4;  /* receiver */
    if (dest_on  && memcmp(addr1, mac_dest,  6) != 0) return;
    if (group_on && memcmp(addr1, mac_group, 6) != 0) return;

    int qos = ((fc16 >> 7) & 1) && ((fc16 & 0x0c) == 0x08);
    off += 24 + (qos ? 2 : 0);
    if (off + 8 > h->caplen) return;
    off += 8;
    ...
    if (ver == 4) {
        ...
        if (ihl < 20 || off + ihl + 8 > h->caplen) return;
        const uint8_t *udp = ip + ihl;
        udp_dst = (udp[2] << 8) | udp[3];
...
"[--udp-port N] [--dest-mac XX:..] [--group-ip A.B.C.D] "
"[--batch N] [--cpu N]\n", argv[0]);
        return 1;
    }
    ...
    for (int i = 5; i < argc; i++) {
        if (!strcmp(argv[i], "--udp-port") && i + 1 < argc) {
            udp_filter = atoi(argv[++i]); continue;
        }
